...
def get_logs(task_id:str,limit:int=200):
    """Return logs ordered oldest-first with stable id for append-only UI."""
    with db() as conn:rows=conn.execute("SELECT id,level,message,created_at FROM task_logs WHERE task_id=%s ORDER BY created_at ASC, id ASC LIMIT %s",(task_id,limit)).fetchall()
    return [{"id":r["id"],"level":r["level"],"message":r["message"],"created_at":_json(r["created_at"])} for r in rows]
...
def random_automations(n:int=15):
    """Kode video_id acak dari automation DB (search_logs) — dipakai suggestion on hover."""
    n=max(1,int(n or 15))
    out=[]
    try:
        con=_autodb();cur=con.cursor()
