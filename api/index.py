...
def task_report(body:ReportBody,request:Request):
    ...
    fields=dict(body.task or {});logs=fields.pop("logs",[]) if isinstance(fields.get("logs"),list) else []
...
def _json_time(v):return v.isoformat() if hasattr(v,"isoformat") else v
...
def automation_save(body:AutomationBody):
    _db_ready();return {"ok":True,"id":upsert_automation(body.name,body.interval_minutes,body.action,body.config,body.enabled)}
